...
router.get("/all/:productId", async (req, res) => {
  try {
    ...
    const foundProduct = await User.findOne({ "products._id": productId }, { "products.$": 1 });

    if (foundProduct && foundProduct.products.length > 0) {
      res.json(foundProduct.products[0]);
    } else {
...
const Product = require('../../models/User.js'); // Import your Product model
...
router.get('/filt', async (req, res) => {
  try {
    ...
    if (productName) {
      filter['products.name'] = { $regex: new RegExp(productName, 'i') };
    }
    ...
    const filteredProducts = await User.find(filter, 'products');
    const allProducts = filteredProducts.reduce((acc, user) => acc.concat(user.products), []);

    res.json(allProducts);
  } catch (error) {
    console.error('Error in /products/filt:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
