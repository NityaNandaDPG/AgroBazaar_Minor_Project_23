...
const Basket = () => {
  ...
  // Calculate GST
  const gst = calculatedTotalPrice * GST_RATE;
    ...
    <div>
      <div className="bg-white rounded p-4 shadow-md">
            ...
            <li key={i} className="border-b border-gray-300 py-2">
              <div className="flex items-center justify-between">
                ...
                <div className="w-1/5 text-right">
                  <span>
                    ₹
                    {(productDetails[item._id]?.price * item.quantity).toFixed(
                      2
                    )}
                  </span>
                </div>
        ...
        <div className="mt-4">
          ...
          <ul>
            {cart.map((item, i) => (
              <li key={i}>
                {productDetails[item._id]?.name || "Product Name Not Found"} - ₹
                {(productDetails[item._id]?.price * item.quantity).toFixed(2)}
              </li>
            ))}
          </ul>
        </div>
        <div className="mt-4">
          ...
          <strong>
            Total Price (including GST): ₹{(totalPrice + gst).toFixed(2)}
          </strong>
        </div>
      </div>
